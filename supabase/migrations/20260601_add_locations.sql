...
-- Eseguire nel SQL Editor di Supabase (o via supabase db push). Idempotente: ri-eseguibile.
...
CREATE INDEX IF NOT EXISTS idx_locations_lang ON locations(lang);
CREATE INDEX IF NOT EXISTS idx_locations_published ON locations(is_published);
CREATE INDEX IF NOT EXISTS idx_locations_position ON locations(lang, position);
CREATE INDEX IF NOT EXISTS idx_location_info_location ON location_info(location_id);
CREATE INDEX IF NOT EXISTS idx_location_info_order ON location_info(location_id, position);

DROP TRIGGER IF EXISTS update_locations_updated_at ON locations;
CREATE TRIGGER update_locations_updated_at
  BEFORE UPDATE ON locations
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
DROP TRIGGER IF EXISTS update_location_info_updated_at ON location_info;
...
DROP POLICY IF EXISTS "Published locations are viewable by everyone" ON locations;
...
DROP POLICY IF EXISTS "Location info viewable for published locations" ON location_info;
...
DROP POLICY IF EXISTS "Public read access for location-media" ON storage.objects;
CREATE POLICY "Public read access for location-media"
  ON storage.objects FOR SELECT USING (bucket_id = 'location-media');
DROP POLICY IF EXISTS "Authenticated users can upload to location-media" ON storage.objects;
...
DROP POLICY IF EXISTS "Authenticated users can update location-media" ON storage.objects;
...
DROP POLICY IF EXISTS "Authenticated users can delete from location-media" ON storage.objects;
